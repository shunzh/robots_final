...
The action selection is even necessary in some environments. The
...
backward alternatively to explore the state space. However, these 
actions may be completely unknown.  If the agent still chooses the
actions uniformly random, then exploration would be low efficient.
...
the action model. The idea is that we divide the learning process into two
phases. The agent learns the actions first and learns the state space
later. In the first phase, it chooses the action with least confidence
...
the knowledge of the action effects. When the robot needs to decide
which is the next action to take, getAction (Line~\ref{asa:getAct}) is
called.
...
state$. Then the robot should finally have the performance of walking
forward and backward in the domain, even without a prior domain
knowledge of the actions.

\section{Experiments}

The plan for the experimental evaluation is that, first, I test ASAMI
in a toy domain. I only naively implemented the action model and the
sensor model. Then, I made ASAMI and Strong ASAMI work on Nao.

Action model and sensor model are both assumed to be cubic functions.
There are four parameters to be estimated for each model. The initial
action model $A_0$ is defined to be $A_0(c) = c$.

Same as \cite{CSJ06}, in the first $T\_START$ iterations, the sensor
model is being learned by the action model, but not used to update
$w_s$. So in the figures $w_s$ doesn't change at the beginning. This
is important for the algorithm to converge.
...
\caption{Testing of ASAMI in the Simulator. $w_s$}
\label{fig:sim}
\end{figure}
...
per second. It's kidnapped at around 3,500th iteration.}
\label{fig:demo}
\end{figure}
...
a 1-dimensional environment. So expecting improvement based on the
